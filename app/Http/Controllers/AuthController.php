        ...
        { 
            ...
            if($count){
                if($count->phone != $request->phone){
                    ...
                    return response()->json(['success' => false, 'message' => 'Enter correct Phone Number. As this Email-ID is already registered with **** *** '. substr($count->phone, -3)]); 
                }

                if($count->email != $request->email){
                    // $success['token'] =  $user->createToken('MyApp')-> accessToken; 
                    return response()->json(['success' => false, 'message' => 'Enter correct Email-ID. As this Phone Number is already registered with ************'. substr($count->email, 10)]); 
                }
                ...
                if($request->phone == '1122334455' &&  $request->email == 'doctor@gmail.com'){
                    ...
                    if(Auth::attempt(['email' => request('email'), 'password' => request('password')])){         
                        ...
                        // ****OTP Send
                        $url = "https://smsapi.24x7sms.com/api_2.0/SendUnicodeSMS.aspx?APIKEY=crDCpeZ4Mh7&MobileNo=".$count->phone."&SenderID=DRTETH&Message=OTP%20to%20login%20to%20DOCTOR-DENTIST%20is%20".$otp.".%20Don't%20share%20the%20OTP%20with%20anyone.%20Keep%20Smiling%20!%20QVsFDk4utYv&ServiceName=TEMPLATE_BASED&DLTTemplateID=1007165533591015003";
                        $ch = curl_init();
                        ...
                        return response()->json(['success' => false, 'error'=>'Please check your Password'], 401); 
                    }
                }
                ...
                $otp = random_int(100000, 999999);
                 ...
                 // ****OTP Send
                $url = "https://smsapi.24x7sms.com/api_2.0/SendUnicodeSMS.aspx?APIKEY=crDCpeZ4Mh7&MobileNo=".$request->phone."&SenderID=DRTETH&Message=OTP%20to%20login%20to%20DOCTOR-DENTIST%20is%20".$otp.".%20Don't%20share%20the%20OTP%20with%20anyone.%20Keep%20Smiling%20!%20QVsFDk4utYv&ServiceName=TEMPLATE_BASED&DLTTemplateID=1007165533591015003";
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, $url);
                curl_setopt($ch, CURLOPT_POST, 0);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    public function login(Request $request){
       if(is_numeric($request->email)){ 
        if(Auth::attempt(['phone' => request('email'), 'password' => request('password')])){         
            ...
            // ****OTP Send
            $url = "https://smsapi.24x7sms.com/api_2.0/SendUnicodeSMS.aspx?APIKEY=crDCpeZ4Mh7&MobileNo=".$request->email."&SenderID=DRTETH&Message=OTP%20to%20login%20to%20DOCTOR-DENTIST%20is%20".$otp.".%20Don't%20share%20the%20OTP%20with%20anyone.%20Keep%20Smiling%20!%20QVsFDk4utYv&ServiceName=TEMPLATE_BASED&DLTTemplateID=1007165533591015003";
                 $ch = curl_init();
                 curl_setopt($ch, CURLOPT_URL, $url);
                 curl_setopt($ch, CURLOPT_POST, 0);
                 curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
      ...
      elseif (filter_var($request->get('email'), FILTER_VALIDATE_EMAIL)) {
        if(Auth::attempt(['email' => request('email'), 'password' => request('password')])){ 
            ...
            $myEmail = 'amitsahoo1418@gmail.com';
             ...
             $otp = random_int(100000, 999999);
                    if($request->phone){
                        $data = User::where('phone', $request->phone)->update(['otp'=> $otp]);
                             $url = "https://smsapi.24x7sms.com/api_2.0/SendUnicodeSMS.aspx?APIKEY=crDCpeZ4Mh7&MobileNo=".$request->phone."&SenderID=DRTETH&Message=OTP%20to%20Change%20Password%20for%20DOCTOR-DENTIST%20is%20".$otp.".%20Don't%20share%20the%20OTP%20with%20anyone.%20Keep%20Smiling%20!%20QVsFDk4utYv&ServiceName=TEMPLATE_BASED&DLTTemplateID=1007165533562446406";
                             $ch = curl_init();
                             curl_setopt($ch, CURLOPT_URL, $url);
                             curl_setopt($ch, CURLOPT_POST, 0);
                             curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                        ...
                        $data = User::where('email', $request->email)->update(['otp'=> $otp]);
                            ...
                            $sub="Here is your Forgot Password OTP";
                            $dataMail = Mail::to($myEmail)->send(new OTPMailForgotPassword($details,$sub));
